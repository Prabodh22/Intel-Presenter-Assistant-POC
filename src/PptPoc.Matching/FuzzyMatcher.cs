...
public static class FuzzyMatcher
{
    ...
    public static (double Score, string MatchedPhrase) Score(string transcriptText, string elementText)
    {
        ...
        // Content words: 3+ chars and not a noise word, OR any numbers (even short ones like "X", "1", "25", "18.95")
        var eContent = eTokens
            .Where(w => (w.Length >= 3 || char.IsDigit(w[0])) && !NoiseWords.Contains(w))
        ...
        foreach (var ew in eContent)
        {
            ...
            // 4. Compact matching to handle hyphens properly (e.g. C-PHY -> CPHY vs C PHY)
            bool hit = false;
            foreach (var tw in tTokens)
            {
                var twCompact = tw.Replace("-", "");
                var ewCompact = ew.Replace("-", "");

                // Match exact compact bounds
                if (!string.IsNullOrEmpty(ewCompact) && twCompact.Equals(ewCompact, StringComparison.OrdinalIgnoreCase))
                {
                    hit = true;
                    break;
                }
