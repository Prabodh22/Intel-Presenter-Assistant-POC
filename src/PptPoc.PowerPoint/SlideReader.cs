...
public class SlideReader : ISlideReader
{
    ...
    private async Task ProcessSingleImageAsync(ImageElement img, int shapeId, byte[] imageBytes, int slideIndex)
    {
        try
        {
            ...
            {
                img.VisualSearchText = BuildVisualSearchText(img);
            }
    ...
    private void ProcessShape(Ppt.Shape shape, SlideSnapshot snapshot, string? groupPrefix, Ppt.Slide slide, float sW, float sH)
    {
        ...
        try
        {
            ...
            // Extract table: emit a table visual and cells as individual text elements
            if (shape.HasTable == Office.MsoTriState.msoTrue)
            {
                // Create a table visual element to group cells
                var tableVisualId = $"{idPrefix}TB{shape.Id}_{snapshot.SlideIndex}";
                var tableAlt = GetAltText(shape);
                var tableTitle = GetTitle(shape);
                var tableNearby = FindNearbyText(shape, snapshot);
                var tableKeywords = InferKeywords(tableAlt, tableTitle, tableNearby);

                var tableVisual = new ImageElement
                {
                    ElementId = tableVisualId,
                    ShapeName = shape.Name,
                    Left = shape.Left,
                    Top = shape.Top,
                    Width = shape.Width,
                    ...
                    AltText = tableAlt,
                    Title = tableTitle,
                    NearbyText = tableNearby,
                    InferredKeywords = tableKeywords,
                    VisualType = "table",
                    Importance = "high",
                    Relationships = new Dictionary<string,string>()
                };

                ClassifyVisualImportance(tableVisual, sW, sH);
                snapshot.ImageElements.Add(tableVisual);
                    ...
                    for (int row = 1; row <= table.Rows.Count; row++)
                    {
                        for (int col = 1; col <= table.Columns.Count; col++)
                        {
                            try
                            {
                                ...
                                var cellElementId = $"{idPrefix}T{shape.Id}_{snapshot.SlideIndex}_R{row}C{col}";
                                snapshot.TextElements.Add(new TextElement
                                {
                                    ...
                                    Words          = TokenizeWords(normalized),
                                    ParagraphIndex = 0,
                                    ParentVisualId = tableVisualId,
                                    ParentVisualReason = "table_cell_routes_to_table"
                                });

                                cellIds.Add(cellElementId);
                            }
                            catch (COMException) { /* merged cell or inaccessible */ }
                        }
                    }
                    // Record relationship: table -> cells
                    if (cellIds.Count > 0)
                        tableVisual.Relationships["cells"] = string.Join("|", cellIds);

                    Log.Debug("Extracted table {ShapeName} on slide {SlideIndex}: {Rows}x{Cols} with {Cells} cells",
                        shape.Name, snapshot.SlideIndex, table.Rows.Count, table.Columns.Count, cellIds.Count);
                }
                catch (COMException ex)
                {
            ...
            if (shape.HasChart == Office.MsoTriState.msoTrue)
            {
                ...
                try
                {
                    ...
                    var perSeriesValues = new List<List<string>>();
                                ...
                                if (valsObj is object[] vals)
                                {
                                    foreach (var v in vals)
                                    {
                                        ...
                                        if (!string.IsNullOrWhiteSpace(numeric))
                                        {
                                            ...
                                            seriesVals.Add(numeric);
                                        }
                                    }
                                }
                                perSeriesValues.Add(seriesVals);
                            }
                            catch (COMException) { }
                        }
                    ...
                    // Populate chart relationships: series names, categories, per-series numeric values
                    try
                    {
                        if (chartVisual.Relationships == null) chartVisual.Relationships = new Dictionary<string,string>();
                        if (seriesNames.Count > 0)
                            chartVisual.Relationships["series_names"] = string.Join("|", seriesNames);
                        if (categories.Count > 0)
                            chartVisual.Relationships["categories"] = string.Join("|", categories);
                        for (int sIdx = 0; sIdx < perSeriesValues.Count; sIdx++)
                        {
                            var vals = perSeriesValues[sIdx];
                            if (vals.Count > 0)
                                chartVisual.Relationships[$"series_{sIdx+1}_values"] = string.Join("|", vals);
                        }
                        // Chart numeric facts already contains the unique numeric facts
                        if (numericFacts.Count > 0)
                            chartVisual.ChartNumericFacts = numericFacts.Distinct(StringComparer.OrdinalIgnoreCase).ToList();
                    }
                    catch { /* best-effort, don't fail preprocessing */ }
                }
                catch (COMException ex)
                {
    ...
    private static void WriteImageDebugArtifact(int slideIndex, ImageElement image)
    {
        try
        {
            var dir = RuntimeArtifactPaths.LogsDirectory;
            ...
            var entry = new
            {
                ...
                searchable_word_count = image.SearchableWords?.Count ?? 0,
                ocr_status = BuildOcrStatus(image),
                visual_type = image.VisualType ?? string.Empty,
                fallback_search_text_preview = string.IsNullOrWhiteSpace(image.VisualSearchText)
                    ? string.Empty
                    : image.VisualSearchText.Substring(0, Math.Min(300, image.VisualSearchText.Length)),
    ...
    private static string BuildVisualSearchText(ImageElement image)
    {
        return string.Join(" ", new[]
            {
                image.GptDescription,
                image.FilteredOcrText,
                image.Title,
                image.AltText,
                image.NearbyText,
                image.VisualType,
                image.VisualSubtype,
                image.LocationLabel,
                image.RegionHorizontal,
                image.RegionVertical,
                image.ChartNumericFacts.Count > 0 ? string.Join(" ", image.ChartNumericFacts) : string.Empty,
                image.InferredKeywords.Count > 0 ? string.Join(" ", image.InferredKeywords) : string.Empty,
                image.Relationships?.Count > 0 ? string.Join(" ", image.Relationships.Values) : string.Empty
            }
            .Where(s => !string.IsNullOrWhiteSpace(s)))
            .Trim();
    }

    private static string BuildOcrStatus(ImageElement image)
    {
        int ocrWords = image.ExtractedWords?.Count ?? 0;
        if (ocrWords > 0)
            return "ok";

        if (!string.IsNullOrWhiteSpace(image.GptDescription))
            return "no_ocr_words_semantic_fallback";

        if (!string.IsNullOrWhiteSpace(image.AltText) || !string.IsNullOrWhiteSpace(image.Title) || !string.IsNullOrWhiteSpace(image.NearbyText))
            return "no_ocr_words_metadata_fallback";

        return "no_ocr_words_no_fallback_text";
    }
}
